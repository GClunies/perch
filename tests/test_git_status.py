...
class TestActionCopy:
    ...
    async def test_copy_file_path(self, tmp_path: Path) -> None:
        """action_copy on a file item copies the absolute path."""
        from unittest.mock import MagicMock
        ...
        with patches[0], patches[1], patches[2], patches[3]:
            ...
            async with app.run_test(size=(120, 40)) as pilot:
                ...
                mock_clipboard.assert_called_once_with(expected_path)
                mock_notify.assert_called_once_with(f"Copied: {expected_path}")

    async def test_copy_commit_hash(self, tmp_path: Path) -> None:
        """action_copy on a commit item copies just the hash."""
        from unittest.mock import MagicMock
        ...
        with patches[0], patches[1], patches[2], patches[3]:
            ...
            async with app.run_test(size=(120, 40)) as pilot:
                ...
                # Navigate to the first commit item
                for i, child in enumerate(panel.children):
                    if isinstance(child, ListItem) and child.name == "commit:aaa111":
                        panel.index = i
                        break
                ...
                mock_notify.assert_called_once_with("Copied: aaa111")

    async def test_copy_nothing_selected(self, tmp_path: Path) -> None:
        """action_copy returns gracefully when no valid item is highlighted."""
        from unittest.mock import MagicMock
    ...
    async def test_copy_disabled_header_is_ignored(self, tmp_path: Path) -> None:
        """action_copy does nothing when a disabled section header is highlighted."""
        from unittest.mock import MagicMock
        ...
        with patches[0], patches[1], patches[2], patches[3]:
            ...
            async with app.run_test(size=(120, 40)) as pilot:
                ...
                # Force index to a section header (index 0 is always a header)
                panel.index = 0
...
class TestUpdateCopyBinding:
    """Tests for GitPanel._update_copy_binding()."""

    async def test_binding_shows_copy_path_for_file(self, tmp_path: Path) -> None:
        """When a file item is highlighted, binding description is 'Copy Path'."""
        from perch.app import PerchApp
        ...
        with patches[0], patches[1], patches[2], patches[3]:
            ...
            async with app.run_test(size=(120, 40)) as pilot:
                ...
                # The last binding for "c" reflects what _update_copy_binding set
                c_bindings = panel._bindings.get_bindings_for_key("c")
                assert len(c_bindings) > 0
                assert c_bindings[-1].description == "Copy Path"

    async def test_binding_shows_copy_hash_for_commit(self, tmp_path: Path) -> None:
        """When a commit item is highlighted, binding description is 'Copy Hash'."""
        from perch.app import PerchApp
    ...
    async def test_binding_survives_refresh_bindings_exception(
        self, tmp_path: Path
    ) -> None:
        """_update_copy_binding handles screen.refresh_bindings() raising."""
        ...
        with patches[0], patches[1], patches[2], patches[3]:
            ...
            async with app.run_test(size=(120, 40)) as pilot:
                ...
                # Navigate to a file item
                for i, child in enumerate(panel.children):
                    if isinstance(child, ListItem) and child.name == "file_a.py":
                        panel.index = i
                        break

                # Make screen.refresh_bindings raise an exception
                ...
                # Binding was still updated before the exception
                c_bindings = panel._bindings.get_bindings_for_key("c")
                assert len(c_bindings) > 0
                assert c_bindings[-1].description == "Copy Path"
